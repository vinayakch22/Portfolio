...
const ProjectCard: React.FC<ProjectCardProps> = (props) => {
  const isLinkValid = !!props.link && props.link !== "#";
  ...
  const cardContent = (
    <motion.div 
      ...
      className={`p-2 rounded-3xl bg-[#F2F2F20C] border-1 border-white/15 shadow-sm shadow-white ${isLinkValid ? "group cursor-pointer" : ""}`}
    >
      <div className="flex flex-col lg:gap-15 gap-5 sm:pt-10 pt-5 lg:px-15 sm:px-12 px-7 bg-gradient-to-b from-neutral-900 to-neutral-500 rounded-2xl overflow-hidden">
        ...
        <div>
          <img
            className={`w-full bg-purple-400 shadow-[0_0_30px_#DDDDDD] rounded-lg translate-y-2 ${isLinkValid ? "group-hover:-rotate-4 group-hover:scale-115 transition-transform duration-400" : ""}`}
  ...
  return (
    <div className="w-full no-expand">
      {cardContent}
    </div>
  );
};
